...
push_markdown:
	...
	cd awesome-selfhosted && git add . && (git diff-index --quiet HEAD || git commit -m "[bot] 从awesome-selfhosted-data构建Markdown $(COMMIT_HASH)")
...
push_html:
	...
	cd awesome-selfhosted-html/ && git add . && (git diff-index --quiet HEAD || git commit --amend -m "[bot] 从awesome-selfhosted-data构建HTML $(COMMIT_HASH)")
